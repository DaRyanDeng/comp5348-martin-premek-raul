...
namespace Bank.Process
{
    class Program
    {
        ...
        {
            using (ServiceHost lSyncHost = new ServiceHost(typeof(BankSynchronousTransferService)))
            using (ServiceHost lAsyncHost = new ServiceHost(typeof(BankAsynchronousTransferService)))
            {
                lSyncHost.Open();
                lAsyncHost.Open();
